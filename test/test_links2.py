# coding: utf-8

"""
    Signalen API

    One of the tasks of a municipality is to maintain public spaces. When citizens have complaints about public spaces they can leave these complaints with the municipality. Signalen (SIG) receives these complaints and is used to track progress towards their resolution. SIG provides an API that is used both by the SIG frontend and external systems that integrate with SIG.

    The version of the OpenAPI document: 2.41.0
    Generated by OpenAPI Generator (https://openapi-generator.tech)

    Do not edit the class manually.
"""  # noqa: E501


import unittest

from signals_api_client.models.links2 import Links2

class TestLinks2(unittest.TestCase):
    """Links2 unit test stubs"""

    def setUp(self):
        pass

    def tearDown(self):
        pass

    def make_instance(self, include_optional) -> Links2:
        """Test Links2
            include_optional is a boolean, when False only required
            params are included, when True both required and
            optional params are included """
        # uncomment below to create an instance of `Links2`
        """
        model = Links2()
        if include_optional:
            return Links2(
                curies = signals_api_client.models._links_1_curies._links_1_curies(
                    href = 'https://api.example.com/signals/relations/', ),
                var_self = signals_api_client.models._links_1_self._links_1_self(
                    href = 'https://api.example.com/signals/v1/private/categories/1', ),
                archives = signals_api_client.models._links_1_archives._links_1_archives(
                    href = 'https://api.example.com/signals/v1/private/signals/1/history/', ),
                sia_status_message_templates = signals_api_client.models._links_2_sia_status_message_templates._links_2_sia_status_message_templates(
                    href = 'https://api.example.com/signals/v1/private/status-messages/category/1', )
            )
        else:
            return Links2(
        )
        """

    def testLinks2(self):
        """Test Links2"""
        # inst_req_only = self.make_instance(include_optional=False)
        # inst_req_and_optional = self.make_instance(include_optional=True)

if __name__ == '__main__':
    unittest.main()
